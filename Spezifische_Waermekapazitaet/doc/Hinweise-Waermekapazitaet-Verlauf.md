...
\end{equation*}
$$
Wärme $\delta Q_{0}$ aus der Umgebung aufnimmt führt dies offensichtlich zu einer Verzerrung der Daten. Man bezeichnet diesen Vorgang als Wärmegang. Sie können diesen Effekt mit Hilfe einer Leermessung ohne elektrische Wärmezufuhr abschätzen und die originale Messung a posteriori (d.h. nachdem die Messung bereits beendet wurde) entsprechend korrigieren. Wir haben eine solche Leermessung, in der Datei [waermegang.csv](https://gitlab.kit.edu/kit/etp-lehre/p2-praktikum/students/-/blob/main/Spezifische_Waermekapazitaet/params/waermegang.csv), für Sie bereitgestellt. 
...
\end{equation}
$$
zur Beschreibung des Verlaufs $T_{0}(t)$ der Leermessung als Funktion der Zeit bewährt, wobei $a_{0},\ b_{0},\ c_{0}$ freie Parameter des Modells sind. Die Ableitung dieses Modells nach $t$ ist 
