...
> Im Atom besetzen die Elektronen der Hülle [Orbitale](https://de.wikipedia.org/wiki/Atomorbital) mit diskreten Energieniveaus. Aufgrund des [Pauli-Prinzips](https://de.wikipedia.org/wiki/Pauli-Prinzip) kann jedes Energieniveau nur durch eine endliche Anzahl von Elektronen besetzt werden. In Molekülen spalten die Energieniveaus z.B. aufgrund zusätzlicher Rotations- und Vibrationsfreiheitsgrade auf. Im Übergang zum Festkörper (FK) nimmt diese Aufspaltung weiter zu, so dass sich (für eine Anzahl von Basisatomen (BA) eines FK in der Größenordnung von $N_{A}$) quasi-kontinuierliche Energiebänder ausbilden. 

Es handelt sich dabei immer noch um eine wenn auch sehr große Zahl diskreter Energieniveaus, die in bestimmten Bereichen sehr eng beieinander liegen. Dieser Übergang ist in **Abbildung 1** schematisch dargestellt:
...
Durch thermische Anregung nimmt der FK Energie aus seiner Umgebung auf, so dass sich Elektronen auch in höheren (erlaubten) Energieniveaus, als im Grundzustand des FK befinden können. Die Wahrscheinlichkeit dafür bei der Temperatur $T$ des FK ein Elektron mit der Energie $E$ anzutreffen ist durch die [Boltzmann-Statistik](https://de.wikipedia.org/wiki/Boltzmann-Statistik) 
$$
\begin{equation*}
p = \frac{1}{Z}e^{-E/k_{\mathrm{B}}T}
\end{equation*}
$$
gegeben, wobei $k_{\mathrm{B}}$ die [Boltzmann-Konstante](https://de.wikipedia.org/wiki/Boltzmann-Konstante) und $Z$ eine Normierungskonstante sind. ℹ️ Für das Produkt $k_{\mathrm{B}}T$ gilt
$$
\begin{equation*}
...
> - Beim **Isolator** ist die Energielücke (engl. *gap*) $E_{g}$ zwischen Valenz- und Leitungsband so groß, dass sie für ein einzelnes Elektron nicht einfach aufgebracht werden kann ($E_{g}>3\ \mathrm{eV}$). Ein Isolator ist i.a. nicht leitend.  
...
**Abbildung 3 (b)** zeigt ein Schema der daraus resultierenden Gitterstruktur, sowie die Konfiguration aus Valenzband, Leitungsband und $E_{F}$.  
...
Mit Hilfe der BCS-Theorie lassen sich alle Eigenschaften konventioneller Supraleiter beschreiben. Das Zustande kommen von HTSL ist jedoch noch nicht vollständig verstanden.

### Widerstandsmessung mit der Vierleiterschaltung

Um die Messung des Widerstands des Supraleiters nicht durch die Widerstände in den nicht-supraleitenden Zuleitungen zu verfälschen erfolgt die Widerstandmessung für **Aufgabe 2.3** mit Hilfe einer [**Vierleitungsschaltung**](https://de.wikipedia.org/wiki/Vierleitermessung), wie in **Abbildung 8 (b)** gezeigt: 
...
- :white_check_mark: Sie können das **Bändermodell** aus dem Übergang vom Atom, über das Molekül, bis zum FK qualitativ erklären. 
- :white_check_mark:  Sie können anhand des Bändermodells den Unterschied zwischen **Leiter, Halbleiter und Isolator** qualitativ erklären. 
- :white_check_mark: Sie können erklären, was die **Fermi-Energie** $E_{F}$ ist.
- :white_check_mark: Sie kennen die **Größenordnung von $k_{\mathrm{B}}T$** bei Zimmertemperatur. 
- :white_check_mark: Sie wissen was ein **Supraleiter vom Typ-1 oder 2** ist.
- :white_check_mark: Sie wissen, was ein **Cooper-Paar** ist.
