# Technische Daten und Inventar für den Versuch **Photoeffekt**

Für den Versuch stehen Ihnen die folgenden Geräte und Apparaturen zur Verfügung:

- Für den Demonstrationsversuch von **Aufgabe 1.1**:
  - Ein statisches Elektrometer (E);
  - Eine Zn-Platte (Zn) und Schmirgelpapier;
  - Eine Metallstabelektrode mit Tonnenfuß;
  - Ein Hochspannungsgerät (mit einem Ausgang für Spannungen zwischen 0 bis $3.5\ \mathrm{kV}$ sowohl positiv als auch negativ gegen Masse und hohem Innenwiderstand, die **Berührung der Anschlüsse ist aufgrund der niedrigen Ströme also ungefährlich**);
- Eine Hochdruck-Quecksilberdampflampe (Hg) mit Vorschaltdrossel;
- Drei Sammellinsen zur optischen Abbildung mit den Brennweiten von $f = 65,\  100,\  200\ \mathrm{mm}$, und jeweils einem Durchmesser von $d = 32\ \mathrm{mm}$;
- Zwei Polarisationsfilter zur kontrollierten Reduktion der Lichtintensität.
- Ein Revolverhalter für sechs Fabry-Perot-Interferenzfilter für $\lambda_{\mathrm{CWL}} = 360,\ 400, \ 440,\ 490,\ 540,\ 590\ \mathrm{nm}$, jeweils mit einer Halbwertsbreite von $\pm10\ \mathrm{nm}$ und einer Durchlässigkeit im Maximum von 45%;
- Eine Vakuumphotozelle mit vollflächig aufgedampfter Kaliumkathode (K) und ringförmiger Platin-Rhodium-Drahtanode (A);
- Eine Messverstärkeranordnung, die wir im Hause hergestellt haben;
- Ein Widerstand mit $R=10\pm1\%\ \mathrm{G\Omega}$ und ein Kondensator mit $C=4.7\pm1\%\ \mathrm{nF}$ zur Signalaufnahme;
- Ein [Raspberry Pi 400](https://gitlab.kit.edu/kit/etp-lehre/p2-praktikum/students/-/blob/main/doc/raspberry-pi-400-product-brief.pdf) (oder höher) zur Aufzeichnung der Daten.
- Eine Zeiss-Schiene mit Reitern;
- Verbindungskabel.
